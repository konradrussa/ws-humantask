...
*/
public class GreaterEqualsOperator extends BinaryExpression implements IOperator {

	public GreaterEqualsOperator(Expression leftExpression, Expression rightExpression) {
	...
	public void eval() {
		...
		if (right instanceof IntegerResult && left instanceof IntegerResult) {
			...
			result = new BooleanResult(l.getValue() >= r.getValue());
			...
			throw new SBQLEvalException("GreaterEqualsOperator.eval: blad typu");
		}
		QRES.getInstance().push(result);
	}

	public void accept(ASTVisitor visitor) {
		visitor.visitGreaterEqualsExpression(this);
	}
